...
export default async function ApYearPage({ params }: PageProps) {
  const { year: yearSlug } = await params
  const yearKey = decodeURIComponent(yearSlug)
  const allExams = await listApExams()
  ...
  return (
    <MobileFrame>
      <Breadcrumbs items={[
        ...
        { name: summary.yearDisplay, href: `/ap/year/${yearSlug}` },
      ]} />
      <JsonLd
        data={collectionPageJsonLd({
          name: `${summary.yearDisplay} 応用情報技術者試験 過去問`,
          description: `応用情報技術者試験 ${summary.yearDisplay} の試験回 ${yearExams.length} 件`,
          url: `${SITE_URL}/ap/year/${yearSlug}`,
      ...
      <h1 className="text-[20px] font-extrabold mb-3">
        {summary.yearDisplay} 応用情報技術者試験 過去問
      </h1>
      ...
      {(summary.prevYear || summary.nextYear) && (
        <nav aria-label="前後の年度" className="flex gap-2 text-[12px] mb-7">
          {summary.prevYear && (
            <Link
              href={`/ap/year/${encodeURIComponent(summary.prevYear)}`}
          ...
          {summary.nextYear && (
            <Link
              href={`/ap/year/${encodeURIComponent(summary.nextYear)}`}
      ...
      <p className="text-[11px] opacity-60 pt-3 border-t border-goukaku-divider">
        ← <Link href="/ap" className="underline">応用情報技術者試験 のトップ</Link>へ
      </p>
    </MobileFrame>
  )
}
