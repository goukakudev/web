...
export async function generateMetadata({
  ...
  return makeMetadata({
    title: `応用情報技術者試験 ランダム${limit}問`,
    description: `応用情報技術者試験の過去問からランダムに${limit}問を出題します。解説・選択肢別解説・ヒント付きで演習できます。`,
    path: "/ap/play/random",
    noindex: true,
  })
}

export default async function ApPlayRandomPage({ searchParams }: PageProps) {
  const { count } = await searchParams
  const limit = parseRandomCount(count)
  ...
  const questionLists = await Promise.all(
    selectedExams.map((e) =>
      listApQuestions(e.exam_id).catch(() => [] as Question[]),
    ),
  )
  ...
  const statsResults = await Promise.all(
    examIdsInSeed.map((examId) => getApExamStats(examId)),
  )
  ...
  return (
    <MobileFrame>
      <h1 className="sr-only">応用情報技術者試験 ランダム{seed.length}問</h1>
      <PlayController
        questions={seed}
        exam={{
          ...VIRTUAL_EXAM,
          title: `応用情報 全試験ランダム ${seed.length}問`,
