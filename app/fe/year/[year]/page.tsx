...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return makeMetadata({
    title: `${display} 基本情報技術者試験 過去問 ${total} 問`,
    description: `基本情報技術者試験 ${display} の過去問 ${yearExams.length} 回・全 ${total} 問の一覧。試験回ごとの出題内訳と解説リンク。`,
    path: `/fe/year/${yearSlug}`,
  })
}
  ...
  return (
    <MobileFrame>
      <Breadcrumbs items={[
        ...
        { name: summary.yearDisplay, href: `/fe/year/${yearSlug}` },
      ]} />
      <JsonLd
        data={collectionPageJsonLd({
          name: `${summary.yearDisplay} 基本情報技術者試験 過去問`,
          description: `基本情報技術者試験 ${summary.yearDisplay} の試験回 ${yearExams.length} 件`,
          url: `${SITE_URL}/fe/year/${yearSlug}`,
      ...
      <h1 className="text-[20px] font-extrabold mb-3">
        {summary.yearDisplay} 基本情報技術者試験 過去問
      </h1>
      ...
      {(summary.prevYear || summary.nextYear) && (
        <nav aria-label="前後の年度" className="flex gap-2 text-[12px] mb-7">
          {summary.prevYear && (
            <Link
              href={`/fe/year/${encodeURIComponent(summary.prevYear)}`}
          ...
          {summary.nextYear && (
            <Link
              href={`/fe/year/${encodeURIComponent(summary.nextYear)}`}
      ...
      <p className="text-[11px] opacity-60 pt-3 border-t border-goukaku-divider">
        ← <Link href="/fe" className="underline">基本情報技術者試験 のトップ</Link>へ
      </p>
    </MobileFrame>
  )
}
