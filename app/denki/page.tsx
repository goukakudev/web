...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return makeMetadata({
    title: "第二種電気工事士 学科試験 過去問 無料 図入り1950問・模試",
    description:
      `無料・登録不要・スマホ最適化。第二種電気工事士 学科試験の過去問を平成21年〜令和8年・${totalText}収録。図入り問題に対応し、順番／ランダム／模試で演習でき、全問に解説と独自の選択肢別解説・ヒント付き。`,
    path: "/denki",
  })
}
...
export default async function DkHomePage() {
  ...
  // API は実施日の降順で返るため、初出順がそのまま新しい年度順になる。
  const denkiYears = [...new Set(exams.map((exam) => exam.year).filter(Boolean))]
  ...
  return (
    <main className="min-h-screen bg-[#f5f2e8] text-[#191815]">
      <div className="mx-auto flex min-h-screen w-full max-w-6xl flex-col px-4 py-5 sm:px-6 lg:px-8">
        ...
        <section className="grid gap-5 py-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:items-stretch">
          <div className="rounded-lg border-2 border-[#191815] bg-[#fffdf6] p-5 shadow-[6px_6px_0_#191815] sm:p-7">
            ...
            <h1 className="mt-3 max-w-2xl text-[34px] font-black leading-[1.08] tracking-normal sm:text-[48px]">
              第二種電気工事士
              <span className="block">学科試験 過去問</span>
            </h1>
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-3">
              {latest ? (
                <Link
                  href={`/denki/play/${latest.exam_id}/q/1`}
              ...
              {latest ? (
                <Link
                  href={`/denki/play/${latest.exam_id}?mode=exam`}
        ...
        <section className="grid gap-4 lg:grid-cols-[1fr_1fr]">
          <div className="rounded-lg border border-[#d8d1bc] bg-white p-5">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-3 lg:grid-cols-1">
              <MenuLink href="#denki-exams" label="年度別" detail="公開回ごとに50問演習" />
              <MenuLink href="/denki/play/random" label="横断ランダム" detail="全試験から20問抽出" />
              <MenuLink href="/denki/guide" label="学習ガイド" detail="計算・配線図・暗記分野の整理" />
            </div>
          </div>
          ...
          <div className="rounded-lg border border-[#d8d1bc] bg-white p-5">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {FOCUS_AREAS.map((area) => (
                <Link
                  key={area.tag}
                  href={`/denki/tag/${tagToSlug(area.tag)}`}
        ...
        <section id="denki-exams" className="mt-8">
          <div className="flex flex-wrap items-end justify-between gap-3">
            ...
            {latest ? (
              <Link href={`/denki/exam/${latest.exam_id}`} className="rounded-lg border border-[#191815]/20 bg-white px-3 py-2 text-[12px] font-extrabold">
                {latestLabel}を見る
              </Link>
            ) : null}
          </div>
        ...
        {denkiYears.length > 0 && (
          <section className="mt-8">
            <p className="text-[11px] font-black tracking-[0.18em] text-[#007c83]">BY YEAR</p>
            <h2 className="mt-1 text-[24px] font-black">年度別に解く</h2>
            <nav aria-label="年度別の過去問" className="mt-4 flex flex-wrap gap-2">
              {denkiYears.map((y) => (
                <Link
                  key={y}
                  href={`/denki/year/${encodeURIComponent(y)}`}
                  className="min-h-10 rounded-lg border border-[#191815]/15 bg-[#fffdf6] px-3 py-2 text-[13px] font-extrabold transition hover:border-[#007c83] hover:bg-[#b8f3f2]"
                >
                  {y}
                </Link>
              ))}
