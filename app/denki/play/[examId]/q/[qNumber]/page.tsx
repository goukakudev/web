...
export async function generateMetadata({
  ...
  try {
    ...
    const description = `第二種電気工事士 学科試験 ${examLabel} 問${n} の問題本文・選択肢・図版・正解・解説・ヒント。${bodyPreview}…`
    const canonical = `/denki/play/${exam.exam_id}/q/${n}`
...
export default async function DkPlayQuestionPage({ params }: PageProps) {
  const { examId, qNumber } = await params
  const n = Number(qNumber)
  ...
  const url = `${SITE_URL}/denki/play/${exam.exam_id}/q/${n}`
  const examLabel = exam.title ?? exam.exam_id
  const examUrl = `/denki/exam/${exam.exam_id}`
  ...
  return (
    <DenkiStudyFrame>
      <Breadcrumbs items={[
        ...
        { name: `問${n}`, href: `/denki/play/${exam.exam_id}/q/${n}` },
      ]} />
      <JsonLd
        data={questionJsonLd({
          name: `${examLabel} 問${n}`,
          ...
          partOfUrl: `${SITE_URL}/denki/exam/${exam.exam_id}`,
        })}
      />
      <h1 className="sr-only">
        第二種電気工事士 学科試験 {examLabel} 問{n}: {stripMd(q.body).slice(0, 80)}
      </h1>
      <PlayController
      ...
      <RelatedQuestions
        ...
        basePath={`/denki/play/${exam.exam_id}/q`}
        examLabel={examLabel}
        variant="denki"
      />
      <QuestionLearningResources examKey="dk" variant="denki" />
      <QuizBottomActions examKey="dk" variant="denki" />
    </DenkiStudyFrame>
  )
}
