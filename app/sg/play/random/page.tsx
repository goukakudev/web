...
export async function generateMetadata({
  ...
  return makeMetadata({
    title: `情報セキュリティマネジメント試験 ランダム${limit}問`,
    description: `情報セキュリティマネジメント試験の過去問からランダムに${limit}問を出題します。解説・選択肢別解説・ヒント付きで演習できます。`,
    path: "/sg/play/random",
    noindex: true,
  })
}
...
export default async function SgPlayRandomPage({ searchParams }: PageProps) {
  ...
  return (
    <MobileFrame>
      <h1 className="sr-only">情報セキュリティマネジメント試験 ランダム{seed.length}問</h1>
      <PlayController
        questions={seed}
        exam={{
          ...VIRTUAL_EXAM,
          title: `情報セキュリティ 全試験ランダム ${seed.length}問`,
