...
export async function generateMetadata({
  ...
  try {
    ...
    return makeMetadata({
      title: `情報セキュリティマネジメント試験 ${examLabel} ${label}`,
      description: `情報セキュリティマネジメント試験 ${examLabel} の${label}ページです。解説・選択肢別解説・ヒント付きで演習できます。`,
      path: `/sg/play/${examId}`,
      noindex: true,
    })
  } catch {
...
export default async function SgPlayPage({ params, searchParams }: PageProps) {
  const { examId } = await params
  const { mode } = await searchParams
  ...
  if (mode === undefined || mode === "sequential") {
    ...
    if (first) redirect(`/sg/play/${examId}/q/${first.q_number}`)
  }
  ...
  return (
    <MobileFrame>
      <h1 className="sr-only">
        情報セキュリティマネジメント試験 {exam.title ?? exam.exam_id} {label}
      </h1>
      <PlayController
        questions={questions}
        exam={exam}
