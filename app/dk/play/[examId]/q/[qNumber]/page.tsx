...
export async function generateMetadata({
  ...
  try {
    ...
    const canonical = `/dk/play/${exam.exam_id}/q/${n}`
...
export default async function DkPlayQuestionPage({ params }: PageProps) {
  ...
  const url = `${SITE_URL}/dk/play/${exam.exam_id}/q/${n}`
  const examLabel = exam.title ?? exam.exam_id
  const examUrl = `/dk/exam/${exam.exam_id}`
  ...
  return (
    <MobileFrame>
      <Breadcrumbs items={[
        ...
        { name: `問${n}`, href: `/dk/play/${exam.exam_id}/q/${n}` },
      ]} />
      <JsonLd
        data={questionJsonLd({
          name: `${examLabel} 問${n}`,
          ...
          partOfUrl: `${SITE_URL}/dk/exam/${exam.exam_id}`,
        })}
      />
      ...
      <RelatedQuestions
        ...
        basePath={`/dk/play/${exam.exam_id}/q`}
        examLabel={examLabel}
      />
      <QuestionLearningResources examKey="dk" />
      <QuizBottomActions examKey="dk" />
    </MobileFrame>
  )
}
