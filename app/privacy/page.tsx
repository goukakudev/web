...
export default function PrivacyPage() {
  return (
    <MobileFrame>
      ...
      <Section title="12. コンテンツの著作権と独自編集について">
        <p>
          当サイト・本アプリに掲載するコンテンツの引用範囲と独自編集部分の境界を、以下のとおり明示します。
        </p>
        <ul className="list-disc pl-5 space-y-1 mt-2">
          <li>
            <strong>引用部分</strong>: 問題文・選択肢・正解番号は、各試験実施団体 (独立行政法人情報処理推進機構 / 一般財団法人不動産適正取引推進機構 / 厚生労働省 ほか) が一般に公表している過去問題に基づきます。引用は、出題内容の検証・学習目的の利用という目的に必要な範囲にとどめ、出典を明示しています (各問題画面および「
            <Link href="/sources" className="underline">出典一覧</Link>
            」参照)。
          </li>
          <li>
            <strong>独自編集部分</strong>: 全体解説、選択肢別解説、ヒント、語句注釈、関連用語、関連問題リンク、分野タグ、難易度区分、メタディスクリプション、本サイト内の解説記事・学習ガイド・FAQ は、すべて合格.dev 編集部が独自に書き起こした二次著作物であり、試験団体の公式見解ではありません。
          </li>
          <li>
            <strong>独自編集物の著作権</strong>: 上記の独自編集部分の著作権は合格.dev に帰属します。引用範囲を超える転載・複製・改変はお断りしています (詳細は{" "}
            <Link href="/terms" className="underline">利用規約</Link>{" "}
            参照)。
          </li>
          <li>
            <strong>AI 利用の方針</strong>: 解説の下書き作成や用語整理に AI を補助的に用いる場合がありますが、最終的な公開コンテンツは人手で校正しています。生成物をそのまま公開する運用は行っていません。
          </li>
          <li>
            <strong>誤り・引用範囲のご指摘</strong>: コンテンツに誤り、または不適切な引用があると思われる場合は、§14 のお問い合わせ窓口までご連絡ください。確認のうえ速やかに対応します。
          </li>
        </ul>
      </Section>
      ...
      <Section title="13. 免責事項">
        <p>
          当サイト・本アプリに掲載する問題および解説は、各試験実施団体が公表する過去問題を元に当サイトが独自に編集したものです。内容の正確性には最善を尽くしていますが、誤りや古い情報が含まれる可能性があります。本サイト・本アプリの利用により生じたいかなる損害についても、運営者は責任を負いません。
        </p>
      </Section>
