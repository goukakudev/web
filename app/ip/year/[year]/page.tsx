...
export default async function IpYearPage({ params }: PageProps) {
  const { year: yearSlug } = await params
  const yearKey = decodeURIComponent(yearSlug)
  const allExams = await listIpExams()
  ...
  return (
    <MobileFrame>
      ...
      <p className="text-[11px] opacity-60 pt-3 border-t border-goukaku-divider">
        ← <Link href="/ip" className="underline">ITパスポート試験 のトップ</Link>へ
      </p>
    </MobileFrame>
  )
}
